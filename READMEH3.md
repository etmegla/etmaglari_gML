...
![Transfer Dictionaries](TransferDictionaries.png)
...
Each node is then given a **one-hot encoded feature vector** from its `cell_type`:
...
4. **Predict** — `pyg.Predict()` runs inference and returns predicted labels alongside actual labels and per-class confidence scores
